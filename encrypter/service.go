...
// I will eventually build on this to make some improvements
// I've already tweaked it a bit to work for me
package main
...
func encryptFile() {
	// Reading plaintext file from generater/passwords.txt
	plainText, err := ioutil.ReadFile("../generater/passwords.txt")
	if err != nil {
	...
	// we don't want that to be there now do we
	err = os.Remove("../generater/passwords.txt")
	if err != nil {
		log.Fatalf("failed to delete your password file: %v", err.Error())
	}
